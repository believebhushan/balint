...
class OperatorDataService{
    ...
    loginOperator = async (status ,locationCode ) =>{
        const loginQuery = query(operatorCollectionRef, where("status", "==", status), where("location_code", "==", locationCode));
    ...
    loginOperatorByCode = async (locationCode) =>{
        const loginQuery = query(operatorCollectionRef,where("location_code", "==", locationCode));
        const response = await getDocs(loginQuery);
        const res = []
        if(response.docs.length>0){
            response.forEach(Operator => {
