...
const Printjob = () => {
    ...
    const getAllJobs = async() => {
        ...
        const this_data = await JobDataService.getAllJobByLocationCode(this_OperatorDetail.location_code);
            ...
            if(jobDetail){ 
                ...
                const papaconfig = {
                    complete: (results, file) => {
                        ...
                        dataRows.forEach(row => {
                            if(row[0].toLowerCase() ===  jobDetail.shape.toLowerCase() &&  row[1] === jobDetail.capsule.toLowerCase()){
                                setDishId('');
                                dish2= row[2];
                                setDishId(dish2);
                            }
                ...
                if(dishId !== null && dishId !== 'undefined'){
                    if(apitoken !== null){
                        ...
                        JobDataService.updateJob(jobDetail.id,jobupdate);
                        getAllJobs();
                    }   
                }
            }
    ...
    const printServer = async(dishId) =>{
    ...
    const getAllPrinter = async() => {
        const this_Operator = localStorage.getItem("getOperator");
        const this_OperatorDetail = this_Operator ? JSON.parse(this_Operator) : null;
        const this_data = await PrinterDataService.getAllPrinterByLocationCode(this_OperatorDetail.location_code);
